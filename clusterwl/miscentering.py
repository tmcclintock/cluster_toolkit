...
def Sigma_mis_at_R(R, Rs, Sigma, M, conc, om, Rmis, delta=200, kernel="gaussian"):
    if kernel == "gaussian":
        return clusterwl._lib.Sigma_mis_g2d_at_R(R, dcast(Rs), dcast(Sigma), len(Rs), M, conc, delta, om, Rmis)

def calc_Sigma_mis_at_R(R, Rs, Sigma, M, conc, om, Rmis, Sigma_mis, delta=200, kernel="gaussian"):
    if kernel == "gaussian":
        return clusterwl._lib.Sigma_mis_g2d_at_R_arr(dcast(R), len(R), dcast(Rs), dcast(Sigma), len(Rs), M, conc, delta, om, Rmis, dcast(Sigma_mis))
